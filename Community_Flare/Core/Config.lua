...
local ADDON_NAME, NS = ...
if (not NS.Loaded or not NS.Loaded["Constants"]) then return end
...
local BattlegroundGroup = {
	...
	args = {
		...
		pvpGearEquipmentSet = {
			...
			desc = L["This will automatically equip your PVP Gear Set inside PVP content."],
		...
		slrNotifications = {
			...
			order = 24,
			name = L["Enable Notifications?"],
			desc = L["This will show you warning messages for critical events around Slayer's Rise."],
			values = {
				[1] = L["None"],
				[2] = L["Local Warning Only"],
			},
			get = function(info) return NS.db.global.slrNotifications end,
			set = function(info, value) NS.db.global.slrNotifications = value end,
		},
	}
}
