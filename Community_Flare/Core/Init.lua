...
function NS:SaveSession()
	...
	if (NS:IsInBattleground() == true) then
		...
		NS.charDB.profile.SLR = NS.CommFlare.CF.SLR or {}
...
-- get current equipment set ID
function NS:GetCurrentEquipmentSetID()
	-- can use equipment sets?
	if (EquipmentSetCanUseEquipmentSets()) then
		-- process all
		for i=0, EquipmentSetGetNumEquipmentSets() do
			-- is equipped?
			local name, iconFileID, setID, isEquipped = NS:GetEquipmentSetInfo(i)
			if (isEquipped) then
				-- return setID
				return setID
			end
